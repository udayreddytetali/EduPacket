      ...
      const SubjectSelection = ({ group, dataType, year, semester }) => {
        ...
        const handleRemoveSubjectConfirm = async (e) => {
          ...
          try {
            ...
            // Defensive: ensure res.data is an array before searching
            ...
            const subjectObj = res.data.find(s => s.name && s.name.trim().toLowerCase() === removeSubjectName.trim().toLowerCase());
            ...
            if (!Array.isArray(updatedRes.data)) {
              setSubjectError('Unexpected response from server after deletion.');
            } else {
